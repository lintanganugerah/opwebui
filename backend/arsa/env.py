...
# ARSA_DIR should be the directory where env.py resides (arsa/)
ARSA_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of ARSA_DIR (backend/)
BACKEND_DIR = ARSA_DIR.parent
...
try:
    ...
    changelog_content = (pkgutil.get_data('arsa', 'CHANGELOG.md') or b'').decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv('DATA_DIR', ARSA_DIR / 'data')).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'arsa_data', 'zip', DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv('DATA_DIR', ARSA_DIR / 'data'))

STATIC_DIR = Path(os.getenv('STATIC_DIR', ARSA_DIR / 'static'))

FONTS_DIR = Path(os.getenv('FONTS_DIR', ARSA_DIR / 'static' / 'fonts'))

FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', BASE_DIR / 'build')).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', ARSA_DIR / 'frontend')).resolve()

...
| `env.py` | Environment bootstrap. Resolves directory paths (`DATA_DIR`, `ARSA_DIR`, `BACKEND_DIR`), loads `.env`, configures logging, Redis, device type (CPU/CUDA/MPS), and exports global constants. ~944 lines. |
...
- This is the `arsa` Python package installed by pip. All imports use `from arsa.<module> import ...`.

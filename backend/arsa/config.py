...
from arsa.internal.db import Base, get_db
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(ARSA_DIR / 'alembic.ini')

        # Set the script location dynamically
        migrations_path = ARSA_DIR / 'migrations'
...
STATIC_DIR = Path(os.getenv('STATIC_DIR', ARSA_DIR / 'static')).resolve()
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (ARSA + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.arsa.svc.cluster.local:11434'
...
MILVUS_COLLECTION_PREFIX = os.environ.get('MILVUS_COLLECTION_PREFIX', 'arsa')
...
QDRANT_COLLECTION_PREFIX = os.environ.get('QDRANT_COLLECTION_PREFIX', 'arsa')
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get('ELASTICSEARCH_INDEX_PREFIX', 'arsa_collections')
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'arsa-index')
